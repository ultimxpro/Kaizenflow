            ...
            return (
                <Tooltip key={id} content={`${user.nom}${isLeader ? ' (Leader)' : ''}`}>
                    <div className="relative">
                        <img src={user.avatarUrl || `https://i.pravatar.cc/150?u=${user.id}`} alt={user.nom} className="w-6 h-6 rounded-full border-2 border-white" />
                        {isLeader && (
...
// **CORRECTION** : Définition de PDCASection à l'extérieur du composant qui l'utilise
...
            } else {
                setDuration(Math.max(1, diffDays)); // Assure une durée minimale de 1
...
                                        <div className="flex">
              _D`                           <input type="number" value={duration} onChange={e => setDuration(parseInt(e.target.value))} min="1" className="p-2 border bg-white border-gray-300 rounded-l w-1/2"/>
...
// **CORRECTION** : Définition de TabButton à l'extérieur du composant qui l'utilise

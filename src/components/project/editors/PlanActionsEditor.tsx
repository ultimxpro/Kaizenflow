...
const DateIndicator = ({ dueDate }: { dueDate: string }) => {
    ...
    let text = "À l'heure";
    if (diffDays < 0) { color = 'text-red-500'; text = `En retard de ${Math.abs(diffDays)}j`; }
    else if (diffDays <= 7) { color = 'text-yellow-500'; text = `Échéance proche (${diffDays}j)`; }
...
const ActionCard = ({ action, onDragStart, onClick }: { action: Action, onDragStart: (e: React.DragEvent, action: Action) => void, onClick: (action: Action) => void }) => {
  ...
  return (
    <div
      ...
      className={`action-card bg-slate-800 border border-slate-700 rounded-lg shadow-sm mb-3 border-l-4 ${config.color} p-3 hover:shadow-md hover:border-slate-600`}
    >
      <div className="flex justify-between items-start mb-2">
        ...
        <span className="text-xs font-medium text-slate-400">{user?.name || 'N/A'}</span>
      </div>
      <h3 className="font-bold text-slate-200 text-sm">{action.title}</h3>
      <div className="mt-3">
...
const ActionModal = ({ isOpen, onClose, onSave, action }: { isOpen: boolean, onClose: () => void, onSave: (action: Action) => void, action: Action | null }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50 p-4">
            <div className="bg-slate-800 border border-slate-700 rounded-lg shadow-xl p-6 md:p-8 w-full max-w-2xl text-slate-200">
                ...
                <form onSubmit={(e) => { e.preventDefault(); onSave(formData as Action); }}>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <input name="title" value={formData.title} onChange={handleChange} placeholder="Titre de l'action" className="p-2 border bg-slate-700 border-slate-600 rounded col-span-2" required />
                        <textarea name="description" value={formData.description} onChange={handleChange} placeholder="Description" className="p-2 border bg-slate-700 border-slate-600 rounded col-span-2 h-24"></textarea>
                        <select name="assignee_id" value={formData.assignee_id} onChange={handleChange} className="p-2 border bg-slate-700 border-slate-600 rounded">
                            <option value="">-- Responsable --</option>
                            {mockUsers.map(u => <option key={u.id} value={u.id}>{u.name}</option>)}
                        </select>
                        <select name="status" value={formData.status} onChange={handleChange} className="p-2 border bg-slate-700 border-slate-600 rounded"><option>À faire</option><option>En cours</option><option>Terminé</option></select>
                        <select name="type" value={formData.type} onChange={handleChange} className="p-2 border bg-slate-700 border-slate-600 rounded"><option value="simple">Action Simple</option><option value="securisation">Sécurisation</option><option value="poka-yoke">Poka-Yoke</option></select>
                        <div><label className="text-sm text-slate-400">Date de début</label><input type="date" name="start_date" value={formData.start_date} onChange={handleChange} className="p-2 border bg-slate-700 border-slate-600 rounded w-full" /></div>
                        <div><label className="text-sm text-slate-400">Date de fin</label><input type="date" name="due_date" value={formData.due_date} onChange={handleChange} className="p-2 border bg-slate-700 border-slate-600 rounded w-full" /></div>
                    ...
                    <div className="mt-6 flex justify-end gap-4">
                        <button type="button" onClick={onClose} className="py-2 px-4 bg-slate-600 rounded hover:bg-slate-500">Annuler</button>
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {Object.entries(columns).map(([type, items]) => (
                <div key={type} className="home-column bg-slate-900/50 border border-slate-800 rounded-lg p-4">
...
const KanbanByPersonView = ({ actions, setActions, onCardClick }) => {
    ...
    return (
        <div>
            <div className="mb-4">
                <select onChange={(e) => setSelectedUser(e.target.value)} value={selectedUser} className="p-2 border bg-slate-700 border-slate-600 rounded shadow-sm text-slate-200">
                    {mockUsers.map(u => <option key={u.id} value={u.id}>{u.name}</option>)}
                </select>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6" onDragEnd={() => setDraggedItem(null)}>
                {Object.entries(columns).map(([status, items]) => (
                    <div key={status} className="kanban-column bg-slate-900/50 border border-slate-800 rounded-lg p-4" onDragOver={(e) => e.preventDefault()} onDrop={(e) => handleDrop(e, status as ActionStatus)} onDragEnter={(e) => e.currentTarget.classList.add('drag-over')} onDragLeave={(e) => e.currentTarget.classList.remove('drag-over')}>
                        <h2 className="font-bold text-slate-300 mb-4 px-1">{status} <span className="text-sm font-normal text-slate-400">{items.length}</span></h2>
...
const MatrixView = ({ actions, onCardClick }) => {
    ...
    return (
        <div className="relative p-8 bg-slate-800 border border-slate-700 rounded-lg shadow-md">
            <div className="absolute top-1/2 -left-4 -translate-y-1/2 -rotate-90 font-bold text-slate-400 tracking-wider">GAIN</div>
            <div className="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-6 font-bold text-slate-400 tracking-wider">EFFORT</div>
            <div className="grid grid-cols-2 grid-rows-2 gap-4">
                <Quadrant title="Quick Win" emoji="🔥" items={matrix['quick-wins']} bgColor="bg-green-300" />
                <Quadrant title="Gros projet" emoji="🗓️" items={matrix['major-projects']} bgColor="bg-yellow-300" />
                <Quadrant title="Effort simple" emoji="👌" items={matrix['fill-ins']} bgColor="bg-blue-300" />
                <Quadrant title="En dernier" emoji="🤔" items={matrix['thankless-tasks']} bgColor="bg-red-300" />
            </div>
        </div>
    );
};
...
const GanttView = ({ actions, onCardClick }) => {
    if (actions.length === 0) return <div className="text-center p-8 text-slate-400">Aucune action à afficher.</div>;
    ...
    return (
        <div className="bg-slate-800 border border-slate-700 p-4 rounded-lg shadow-md overflow-x-auto">
            <h2 className="text-xl font-bold mb-4 text-slate-200">Chronologie Gantt</h2>
            <div className="relative pt-4" style={{ minWidth: '800px', height: `${sortedActions.length * 3.5}rem` }}>
                ...
                {sortedActions.map((action, index) => {
                    ...
                    return (
                        <div key={action.id} className="absolute w-full h-10 flex items-center" style={{ top: `${index * 3.5}rem` }}>
                            <div className="w-1/4 pr-4 text-sm font-medium truncate text-slate-300">{action.title}</div>
...
export const PlanActionsEditor = () => {
  ...
  const TabButton = ({ active, onClick, children, icon }: { active: boolean, onClick: () => void, children: React.ReactNode, icon: string }) => (
    <button onClick={onClick} className={`py-2 px-4 rounded-md text-sm font-medium flex items-center gap-2 transition-colors ${active ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-300 hover:bg-slate-700'}`}>
      <i className={`fa-solid ${icon}`}></i> {children}
    </button>
  );

  return (
    <div className="p-4 sm:p-6 md:p-8 bg-slate-900 text-slate-200 min-h-screen w-full h-full">
      <header className="flex flex-wrap justify-between items-center mb-6 gap-4">
        <h1 className="text-3xl font-bold text-slate-100">Plan d'Actions Kaizen</h1>
        <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 bg-slate-800 border border-slate-700 p-1 rounded-lg shadow-sm">
            ...
            <button onClick={() => openModal()} className="py-2 px-4 bg-green-600 text-white rounded-lg shadow-md hover:bg-green-700 flex items-center gap-2">
